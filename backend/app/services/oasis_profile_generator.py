...
将Zep图谱中的Entities转换为OASIS模拟平台所需的Agent Profile格式

优化改进：
1. 调用Zep检索功能二次丰富Nodes信息
2. 优化提示词生成非常详细的人设
3. 区分 人Entities和抽象群体Entities
"""
...
class OasisProfileGenerator:
    ...
    将Zep图谱中的Entities转换为OASIS模拟所需的Agent Profile
    ...
    #  人类型Entities (需要生成具体人设）
    INDIVIDUAL_ENTITY_TYPES = [
        "student", "alumni", "professor", "person", "publicfigure", 
        "expert", "faculty", "official", "journalist", "activist"
    ]
    
    # 群体/机构类型Entities (需要生成群体代表人设）
    GROUP_ENTITY_TYPES = [
    ...
    def generate_profile_from_entity(
        ...
        """
        从ZepEntities生成OASIS Agent Profile
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        """
        使用Zep图谱混合搜索功能获取Entities相关的丰富信息
        
        Zep没有内置混合搜索接口，需要分别搜索edges和nodes然后合并结果。
        使用并行请求同时搜索，提高效率。
        
        Args:
            entity: EntitiesNodes对象
        ...
        def search_nodes():
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    last_exception = e
                    if attempt < max_retries - 1:
                        logger.debug(f"ZepNodes搜索第 {attempt + 1} 次失败: {str(e)[:80]}, 重试中...")
                        time.sleep(delay)
                        delay *= 2
                    else:
                        logger.debug(f"ZepNodes搜索在 {max_retries} 次尝试后仍失败: {e}")
            return None
        ...
        try:
            ...
            if results["node_summaries"]:
                context_parts.append("相关Entities:\n" + "\n".join(f"- {s}" for s in results["node_summaries"][:10]))
            results["context"] = "\n\n".join(context_parts)
            
            logger.info(f"Zep混合检索完成: {entity_name}, 获取 {len(results['facts'])} 条事实, {len(results['node_summaries'])}  相关Nodes")
    ...
    def _build_entity_context(self, entity: EntityNode) -> str:
        ...
        if entity.attributes:
            ...
            if attrs:
                context_parts.append("### Entities属性\n" + "\n".join(attrs))
            ...
            for edge in entity.related_edges:  # 不限制数量
                ...
                if fact:
                    ...
                    if direction == "outgoing":
                        relationships.append(f"- {entity.name} --[{edge_name}]--> (相关Entities)")
                    else:
                        relationships.append(f"- (相关Entities) --[{edge_name}]--> {entity.name}")
            
            if relationships:
                context_parts.append("### 相关事实和Edges\n" + "\n".join(relationships))
        
        # 3. 添加RelationsNodes的详细信息
        if entity.related_nodes:
            ...
            if related_info:
                context_parts.append("### RelationsEntity Info\n" + "\n".join(related_info))
    ...
    def _fix_truncated_json(self, content: str) -> str:
        ...
        # 检查是否有未闭合的字符串
        # 简单检查：如果最后一 引号后没有逗号或闭合括号，可能是字符串被截断
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """Get system prompt"""
        base_prompt = "You are an expert at generating social media user personas. Generate detailed, realistic personas for opinion simulation that faithfully reflect real-world context. You MUST return valid JSON format. All string values must not contain unescaped newline characters."
    ...
    def _build_individual_persona_prompt(
        ...
        return f"""Generate a detailed social media user persona for this entity, faithfully reflecting real-world context.
...
2. persona: Detailed persona description (2000 words, plain text), including:
   ...
   - Personal memory (important part of persona: describe this individual's connection to events and their past actions/reactions)
3. age: Age as integer (must be an integer)
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        """Build detailed account persona prompt for group/institutional entities"""
        
        attrs_str = json.dumps(entity_attributes, ensure_ascii=False) if entity_attributes else "None"
        context_str = context[:3000] if context else "No additional context"
        
        return f"""Generate a detailed social media account profile for this institution/group entity, faithfully reflecting real-world context.
...
2. persona: Detailed account profile description (2000 words, plain text), including:
   - Institution basic info (official name, nature of organization, founding background, primary functions)
   ...
   - Institutional memory (important part of persona: describe this institution's connection to events and its past actions/reactions)
...
5. mbti: MBTI type describing account style (e.g., ISTJ for conservative/rigorous)
6. country: Country name in English (e.g., "China", "United States")
...
- persona must be a coherent block of text with no newline characters
- {get_language_instruction()} (gender field must use English: "other")
- age must be the integer 30, gender must be the string "other"
- The institutional account's communication must match its identity and positioning"""
    ...
    def generate_profiles_from_entities(
        ...
        """
        批量从Entities生成Agent Profile（支持并行生成）
        ...
        Args:
            ...
            realtime_output_path: 实时写入的文件路径（如果提供，每生成一 就写入一次）
        ...
        def generate_single_profile(idx: int, entity: EntityNode) -> tuple:
            ...
            try:
                ...
                logger.error(f"生成Entities {entity.name} 的人设失败: {str(e)}")
                # 创建一 基础profile
        ...
        print(f"\n{'='*60}")
        print(f"开始生成Agent人设 - 共 {total}  Entities，并行数: {parallel_count}")
        print(f"{'='*60}\n")
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=parallel_count) as executor:
            ...
            for future in concurrent.futures.as_completed(future_to_entity):
                ...
                try:
                    ...
                    logger.error(f"处理Entities {entity.name} 时发生异常: {str(e)}")
                    with lock:
                        completed_count[0] += 1
