...
同时运行Twitter和RedditSimulation，读取相同的Config文件
...
日志结构:
    ...
    ├── simulation.log       # 主Simulation进程日志
...
if os.path.exists(_env_file):
    ...
    print(f"已加载环境Config: {_env_file}")
else:
    _backend_env = os.path.join(_backend_dir, '.env')
    if os.path.exists(_backend_env):
        load_dotenv(_backend_env)
        print(f"已加载环境Config: {_backend_env}")
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    如果Config了加速 LLM，并行Simulation时可以让不同平台使用不同的 API 服务商，提高并发能力。
    ...
    if not os.environ.get("OPENAI_API_KEY"):
        raise ValueError("缺少 API Key Config，请在项目根目录 .env 文件中设置 LLM_API_KEY")
...
async def main():
    ...
    parser.add_argument(
        ...
        type=str, 
        required=True,
        help='Config文件路径 (simulation_config.json)'
    )
    parser.add_argument(
        '--twitter-only',
        action='store_true',
    ...
    parser.add_argument(
        ...
        action='store_true',
        default=False,
        help='SimulationComplete后立即关闭环境，不进入等待命令模式'
    )
    ...
    if not os.path.exists(args.config):
        print(f"Error: Config文件不存在: {args.config}")
        sys.exit(1)
    ...
    log_manager.info(f"Config文件: {args.config}")
    ...
    log_manager.info(f"Simulation参数:")
