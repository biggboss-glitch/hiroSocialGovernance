...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    const relMatch = text.match(/Edge Chain:\s*(\d+)/)
    ...
    const relSection = text.match(/### \*\*Edge Chain\*\*\n([\s\S]*?)(?=\n###|$)/)
    if (relSection) {
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    const activeMatch = text.match(/Current Active Fact:\s*(\d+)/)
    ...
    const activeSection = text.match(/### 【Current Active Fact】[\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (activeSection) {
