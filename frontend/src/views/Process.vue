<template>
  <div class="process-page">
    <!-- Top Navigation -->
    <nav class="navbar">
      <div class="nav-brand" @click="goHome">HIRO</div>
            ...
            <div class="phase-detail">
              <div class="detail-section">
                ...
                <div class="detail-content">
                  上传文档后，LLM分析文档内容，自动生成适合舆论模拟的本体结构（Entity Types + Edges类型）
                </div>
              </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">API Description</div>
                <div class="detail-content">
                  基于生成的本体，将文档分块后调用 Zep API 构建知识图谱，提取Entities和Edges
                </div>
              </div>
              ...
              <div class="detail-section" v-if="graphData">
                ...
                <div class="build-result">
                  <div class="result-item">
                    ...
                    <span class="result-label">EntitiesNodes</span>
                  </div>
                  <div class="result-item">
                    <span class="result-value">{{ graphData.edge_count }}</span>
                    <span class="result-label">Edges边</span>
                  </div>
                  <div class="result-item">
...
const goToNextStep = () => {
  alert('Env Setup feature under development...')
}
...
const handleNewProject = async () => {
  ...
  if (!pending.isPending) {
    error.value = 'No pending simulation request, please return to home page'
    loading.value = false
    return
  }
...
const startBuildGraph = async () => {
  try {
    ...
    error.value = '启动Graph Build失败: ' + (err.message || '未知错误')
    buildProgress.value = null
  }
}
  ...
  try {
    ...
    if (response.success) {
      ...
      if (task.status === 'completed') {
        ...
        stopPolling()
        stopGraphPolling()
        error.value = 'Graph Build失败: ' + (task.error || '未知错误')
        buildProgress.value = null
      }
    }
  } catch (err) {
